...
namespace StudentBack.Controllers
{
    ...
    public class HighClassController : Controller
    {
        ...
        {
            HighClass _HighClass = repo.GetByID(int.Parse(Id));
            ViewBag.Id = _HighClass.Id;
            ViewBag.title = _HighClass.title;
            ViewBag.Description = _HighClass.Description;
            ViewBag.InitImage = _HighClass.Image;
            ViewBag.Course = _HighClass.Course;
            return View();
        }
        ...
        public ActionResult Edit(FormCollection form, IEnumerable<HttpPostedFileBase> Images)
        {
                    ...
                    if (Image != null)
                    {
                        ...
                        if (InitCount == 0)
                        {
                            foreach (var x in form["InitImage"].Split(','))
                            {
                                if (x != "")
                                {
                                    string strPath1 = string.Format("~/Image/High_Lesson/{0}", x);
            ...
            _HighClass.Id = int.Parse(form["Id"]);
            _HighClass.title = form["title"];
            _HighClass.Description = form["Description"];
            _HighClass.Course = form["Course"];
            _HighClass.CreateTime = DateTime.Now;
